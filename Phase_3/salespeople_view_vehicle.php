...
<form method="post" action="perform_sale.php?id=<?php echo $_GET['id'];?>">
<table>
<style>
table {
  border: 1px solid black;
...
$sql = "SELECT V.VIN, type_name,model_year, manu_name,model_name,color,sales_price,mileage,V.optional_desc FROM inventoryvehicle AS I,Vehicle AS V, Vehicle_Color WHERE  V.VIN ='$ID' AND I.VIN=V.VIN AND I.VIN=Vehicle_Color.VIN";
 $result2 = $mydb->query($sql);
  ...
  .$row2['model_name']."</td><td>".$row2['color']."</td><td>".$row2['mileage']."</td><td>".$row2['sales_price']."</td><td>".$row2['optional_desc']."</td><td>"."<a href='perform_sale.php'><button>Sell this vehicle</button></a>|";  
    } //END of while   
